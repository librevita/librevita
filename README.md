...
Migrations are organized by domain (`db/migrations/00001`..`00009`) and the schema used by sqlc lives in
...
Generated repository code is not a source artifact. Edit the SQL under `db/schema` or `db/query`, then run `earthly
+generate` when local generated files are needed.

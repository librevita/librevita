...
(`db/schema/schema.sql`). The schema file is a build artifact, not versioned; it is regenerated by `earthly +generate`
...
`db/migrations` and the queries under `db/query`, then run `earthly +generate` when local generated files are needed.

...
acme:
  ...
  storage:
    backend: keystore # keystore (default, bbolt creates <data_dir>/acme.db) or file
    dir: ./data/acme # used when backend: file (default: <data_dir>/acme)
  dns:
...
| `--acme-storage-backend`             | `LIBREVITA_ACME_STORAGE_BACKEND`             | ACME persistence backend: `keystore` (default, bbolt creates dedicated `acme.db`) or `file`                                                                  |

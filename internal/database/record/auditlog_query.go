...
func (_q *AuditLogQuery) QueryClinic() *ClinicQuery {
...
func (_q *AuditLogQuery) WithClinic(opts ...func(*ClinicQuery)) *AuditLogQuery {
	query := (&ClinicClient{config: _q.config}).Query()
...
func (_q *AuditLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditLog, error) {
	...
	if query := _q.withClinic; query != nil {
		if err := _q.loadClinic(ctx, query, nodes, nil,
			func(n *AuditLog, e *Clinic) { n.Edges.Clinic = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AuditLogQuery) loadClinic(ctx context.Context, query *ClinicQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *Clinic)) error {
	ids := make([]ident.ClinicID, 0, len(nodes))
	nodeids := make(map[ident.ClinicID][]*AuditLog)
	for i := range nodes {
		if nodes[i].ClinicID == nil {
			continue
		}
		fk := *nodes[i].ClinicID
...
func (_q *AuditLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withClinic != nil {
			_spec.Node.AddColumnOnce(auditlog.FieldClinicID)
		}
	}

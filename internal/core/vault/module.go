...
func NewKeyVaultFromConfig(cfg *config.Config, lc fx.Lifecycle, logger log.Logger) (crypto.KeyVault, error) {
	...
	case "bbolt", "":
		...
		logger.Info("initializing bbolt key vault adapter",
		...
		lc.Append(fx.Hook{
			OnStop: func(ctx context.Context) error {
				logger.Info("closing bbolt key vault adapter")
				return v.Close()
			},
		})
		return v, nil
	...
	case "nats":
		...
		logger.Info("initializing nats jetstream key vault adapter",
			log.String("url", url),
		...
		lc.Append(fx.Hook{
			OnStop: func(ctx context.Context) error {
				logger.Info("closing nats key vault adapter")
				return v.Close()
			},
		})
		return v, nil
	...
	case "etcd":
		...
		logger.Info("initializing etcd v3 key vault adapter",
			log.String("endpoints", endpoints),
		...
		lc.Append(fx.Hook{
			OnStop: func(ctx context.Context) error {
				logger.Info("closing etcd key vault adapter")
				return v.Close()
			},
		})
		return v, nil
	...
	case "hashicorp", "hashicorp_vault", "openbao":
		...
		logger.Info("initializing hashicorp vault key vault adapter",
			log.String("address", address),
			log.String("mount", mount),
		)
		...
		lc.Append(fx.Hook{
			OnStop: func(ctx context.Context) error {
				logger.Info("closing hashicorp key vault adapter")
				return v.Close()
			},
		})
		return v, nil

...
// db/query/audit.sql).
//
// The trail is global (no clinic_id): the tenant model is single-clinic
// per installation (ADR-0001), so every event belongs to the only
// clinic. A future multi-clinic mode must version the BLAKE2b chain
// payload before adding the column.
package audit
